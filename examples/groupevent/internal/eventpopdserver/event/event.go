...
func NewAEvent(ev EventM) error {
	...
	if err := newAEvent(ev.Name, ev.StartDate, ev.ExpiredOn, ev.MemberCountLimit, ev.Address, ev.Desc); err != nil {
...
func JoinAEvent(eventId string, m MemberM) error {
	...
	if err := joinAEvent(eventId, m.Name, m.Gm, m.StudentId, m.College, m.Level, m.Profession); err != nil {
